...
const { Engine, Events, Render, Resolver, World, Body, Bodies, Mouse, MouseConstraint, Vector } = require('matter-js')
...
const start = (socket) => {
  ...
  // socket.on('numberPressed', (desiredForce) => {
  ...
  Events.on(engine, 'afterUpdate', () => {
    ...
    // console.log('afterupdate', gameState)
    // socket.emit('engineUpdated', engine.world.bodies[0]);
    gameEvents.emit('gameStateUpdated', gameState);
  });

  World.add(engine.world, [...balls, ...cushions]);

  // Engine.run(engine);
  return setInterval(() => Engine.update(engine, ENGINE_DELTA_TIME_MS), ENGINE_DELTA_TIME_MS);
